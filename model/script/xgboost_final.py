...
features = data.drop(columns=['pts_H', 'pts_A', 'referee_id', 'point_diff', 'home_team', 'away_team', 'referee_name', 'winner', 'season', 'date'])
...
# Creazione del modello XGBRegressor con i migliori parametri trovati
...
joblib.dump(preprocessor, 'preprocessor_xgboost.joblib')
...
plt.figure(figsize=(12, 12))  # Aumentiamo le dimensioni del grafico
plt.barh(importance_df['Feature'], importance_df['Importance'], color='skyblue')
plt.xlabel('Importance')
plt.ylabel('Feature')
plt.title('Feature Importance - XGBoost Regressor')
plt.gca().invert_yaxis()
plt.xticks(rotation=45, fontsize=8)  # Riduciamo la dimensione del font per le etichette
plt.yticks(fontsize=8)  # Riduciamo la dimensione del font per le etichette delle feature
plt.show()
...
random_predictions = best_xgb.predict(random_samples)
...
predicted_diff, actual_diff = predict_game(game_id_to_predict, best_xgb, preprocessor)
